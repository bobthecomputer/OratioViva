...
class RotaryEmbedding(nn.Module):
    ...
    def _update_cos_sin_cache(self) -> None:
        t = torch.arange(
            self.max_seq_len, device=self.inv_freq.device, dtype=self.inv_freq.dtype
        )
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.q_proj = nn.Linear(
            dim, self.num_query_heads * self.head_dim, bias=False, device=device
        )
        self.k_proj = nn.Linear(
            dim, self.num_kv_heads * self.head_dim, bias=False, device=device
        )
        self.v_proj = nn.Linear(
            dim, self.num_kv_heads * self.head_dim, bias=False, device=device
        )
        self.o_proj = nn.Linear(
            self.num_query_heads * self.head_dim, dim, bias=False, device=device
        )
        eps = config.model.normalization_layer_epsilon
        self.q_norm = nn.RMSNorm(
            self.head_dim, eps=eps, dtype=torch.float32, device=device
        )
    ...
    def _project_heads(
        self, layer: nn.Linear, x: torch.Tensor, heads: int
    ) -> torch.Tensor:
...
class MultiStreamEmbedding(nn.Module):
    ...
    def forward(
        self, main_inputs: torch.Tensor, second_inputs: torch.Tensor
    ) -> torch.Tensor:
...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        self.activation_fns = [
            _get_activation(activations[0]),
            _get_activation(activations[1]),
        ]
