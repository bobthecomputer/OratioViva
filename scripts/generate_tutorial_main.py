#!/usr/bin/env python3
"""
OratioViva Tutorial Audio Generator - Main Python Environment
...
class Dia2TTS:
    """Simple Dia2 TTS wrapper using main Python environment."""
    ...
    def load(self):
        ...
        model_path = self.models_dir / "nari-labs_Dia2-2B"
    ...
    def synthesize(self, text, voice_id="dia2_2b_en", speed=1.0):
...
async def generate_segment(segment_id, text, job_id, tts):
    ...
    # Generate audio
    audio = tts.synthesize(enhanced_text, voice_id=TUTORIAL_VOICE, speed=TUTORIAL_SPEED)
    ...
    # Assume 22050 Hz sample rate for Dia2
    sample_rate = 22050
    ...
    return Result(str(output_path), duration)
...
async def generate_tutorial_audio():
    ...
    # Initialize TTS
    tts = Dia2TTS(MODELS_DIR)
    try:
        tts.load()
    except Exception as e:
        print(f"Error loading Dia2 model: {e}")
        print("Make sure the model is downloaded at:")
        print(f"  {MODELS_DIR / 'nari-labs_Dia2-2B'}")
        return False
    ...
    for i, (segment_id, text, target_duration) in enumerate(TUTORIAL_SEGMENTS):
        ...
        try:
            result = await generate_segment(segment_id, text, job_id, tts)
